...
- [[#use-of-the-modes-normalinsert][Use of the modes (normal/insert).]]
- [[#fast-heading-ordering][Fast heading ordering.]]
- [[#highlight-mode][Highlight mode.]]
- [[#folding-heading][Folding heading.]]
- [[#indirect-narrow-buffer][Indirect narrow buffer.]]
...
- =h= Move left.
- =l= Move right.
- =w= Move to the next word - in the begging of it.
- =e= Move to the next word - in the end of it.
- =b= Move to the previous word.
- =Ctrl b= Move up the page.
- =Ctrl f= Move down the page.
- =g g= Go to the first line of the document.
- =Shift g= Go to the last line of the document.
* Use of the modes (normal/insert).
- =i= Change to insert mode - After the cursor mark.
- =a= Change to insert mode - Before the cursor mark.
- =o= Add a line beneath the actual line.
- =Ctrl o= Add a line upside of the actual line.
- =Ctrl g= =Esc= =Enter j k= Exit insert mode.
- =x= Is like delete key.
- =u= Key for undo the changes.
- =Ctrl r= Key for redo the changes.
- =d d= Delete the current line.
- =p= Paste the content of the clipboard.
- =y= Copy the current word.
- =Shift a= Go to the begging of the actual line and enter insert mode .
- =Shift i= Go to the enf fo the actual line and enter insert mode.
* Fast heading ordering.
- =alt j= =alt i= Moves up or down the current line .
- =alt h= =alt l= Moves left or left the current line .
* Highlight mode.
- =v= Enters highlight mode.
- =v e= Highlight and goes to the end of the next word.
- =v w= Highlight and goes to the begging of the next word.
- =Ctrl v= Enter visual mode in block state.
- =Ctrl v + Ctrl i + "insert text"= Enter visual mode in block state, then goes to the begging of the selected lines and insert the text, after exit the insert mode it is going to add the inserted text to all the highlighted lines.
- =Shift b= Highligh all the line.
* Folding heading.
- =Tab= Fold the current selected heading.
- =Shift Tab= Fold all the headings in the document.
- =Shift Tab Shift Tab= Unfold all the heading in the current document.
* Indirect narrow buffer.
- =SPC b c= Opens a document in the indirect buffer mode(it opens in no real time changes, it means that it is like two same files).
- =SPC m s n= It swallows all the document and only show the current paragraph lines.
- =SPC m s N= Comes to the normal mode of the =SPC m s n= mode.
...
* Code
 - =C-c .= - c-set-style -- Set a the tabulation style on the file.
...
- [[https://youtu.be/WyQ4zEo8_Ig?si=8vLz4grQuVZteUjQ][Link to a youtube basics of emacs]]
